        ...
        stage('Deploy to AWS EC2') {
            steps {
                sshagent(['aws-ec2-key']) {
                    ...
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@51.20.73.168 "mkdir -p ~/campus-event-engine && sudo apt-get update && sudo apt-get install -y docker.io docker-compose"'
                    
                    // 2. Copy your project files to the AWS server
                    sh 'scp -o StrictHostKeyChecking=no -r ./* ubuntu@51.20.73.168:/home/ubuntu/campus-event-engine'
                    
                    // 3. Start the Docker containers on AWS
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@51.20.73.168 "cd ~/campus-event-engine && sudo docker-compose down && sudo docker-compose up -d --build"'
                }
            }
        }
        ...
        stage('Verify Running Containers') {
            steps {
                sshagent(['aws-ec2-key']) {
                    ...
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@51.20.73.168 "sudo docker ps --filter \'name=campus-event-engine\'"'
                }
            }
        }
    }
